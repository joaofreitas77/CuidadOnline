...
import { AuthLayoutProvider, useAuthStyles } from './src/auth-layout';
import MaterialCommunityIcons from '@expo/vector-icons/MaterialCommunityIcons';

function Icon({ name, size = 20, color }) { const { C } = useTheme(); return <Feather name={name} size={size} color={color ?? C.blue}/>; }
function Button({ children, onPress, secondary, style }) {
  const { s: themeStyles, C } = useTheme();
  const s = useAuthStyles(themeStyles);
...
function Link({ children, onPress }) {
  const { s: themeStyles, C } = useTheme();
  const s = useAuthStyles(themeStyles); return <Pressable accessibilityRole="button" onPress={onPress} style={s.link}><Text style={s.linkText}>{children}</Text></Pressable>; }
function Brand({ light }) {
  const { s: themeStyles, C } = useTheme();
  const s = useAuthStyles(themeStyles); return <View style={s.row}><View style={[s.logo, light && { backgroundColor: '#FFFFFF25' }]}><Icon name="shield" color="white" size={24}/></View><Text style={[s.brand, light && { color: 'white' }]}>Cuidad<Text style={{ color: light ? '#A9DAFF' : C.blue }}>O</Text><Text style={{ fontWeight: '400' }}>nline</Text></Text></View>; }
function Shield({ hero }) {
  const { s: themeStyles, C } = useTheme();
  const s = useAuthStyles(themeStyles); return <View style={hero ? s.heroArt : s.art}><View style={s.orbit}><View style={s.orbitInner}><View style={s.shield}><Icon name="shield" size={72}/><View style={{ position: 'absolute' }}><Icon name="check" size={30}/></View></View></View></View><View style={s.artTag}><Icon name="check-circle" size={17} color={C.green}/><Text style={{ color: C.green, fontSize: 14, fontWeight: '600' }}>Conhecimento que protege</Text></View></View>; }
...
function Dashboard({ user, onExit }) {
  ...
  return <View style={s.app}>
    ...
    <View style={{ flex: 1 }}><View style={[s.topbar, !wide && { paddingHorizontal: 20 }]}>{wide ? <Text style={s.small}>Seu espaço de aprendizagem</Text> : <Brand/>}<View style={s.row}><Pressable accessibilityRole="button" accessibilityLabel={mode === 'dark' ? 'Ativar tema claro' : 'Ativar tema escuro'} aria-pressed={mode === 'dark'} onPress={toggleTheme} style={({ hovered, pressed }) => [s.themeToggle, hovered && { borderColor: C.blue }, pressed && { opacity: .7 }]}><MaterialCommunityIcons name={mode === 'dark' ? 'lightbulb-on-outline' : 'lightbulb-outline'} size={23} color={C.blue}/></Pressable>{width > 450 && <View style={s.badge}><Text style={s.tiny}>Demonstração</Text></View>}{wide && <Text style={s.label}>{user.split(' ')[0]}</Text>}<View style={s.avatar}><Text style={{ color: C.blue, fontWeight: '700' }}>{user[0].toUpperCase()}</Text></View></View></View>
...
function AppContent({ screen, setScreen }) {
  const { C, mode } = useTheme();

  const [user, setUser] = useState('Visitante');
  return <View style={{ flex: 1, paddingTop: Platform.OS === 'android' ? 30 : Platform.OS === 'ios' ? 54 : 0, backgroundColor: C.bg }}><StatusBar style={mode === 'dark' ? 'light' : 'dark'}/>{screen === 'dashboard' ? <Dashboard user={user} onExit={() => { setUser('Visitante'); setScreen('login'); }}/> : <AuthLayoutProvider register={screen === 'register'}><Auth key={screen} register={screen === 'register'} navigate={setScreen} onEnter={name => { setUser(name); setScreen('dashboard'); }}/></AuthLayoutProvider>}</View>;
}

export default function App() {
  const [screen, setScreen] = useState('login');
  return <ThemeProvider forceLight={screen === 'login' || screen === 'register'}><AppContent screen={screen} setScreen={setScreen}/></ThemeProvider>;
}
